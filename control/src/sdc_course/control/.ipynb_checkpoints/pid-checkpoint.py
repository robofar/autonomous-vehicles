...
class PIDLongitudinalController:
    ...
    def _pid_control(self, target_velocity_ms, current_velocity_ms):
        ...
        acceleration_throttle = np.tanh(acceleration)
        return acceleration_throttle
...
class PIDLateralController:
    ...
    def lateral_error(waypoints, vehicle_transform):
        # Extract current vehicle position
        vehicle_position = np.array([vehicle_transform.x, vehicle_transform.y])

        # Find the nearest waypoint to the current vehicle position
        nearest_waypoint = min(waypoints, key=lambda waypoint: np.linalg.norm([waypoint.x - vehicle_position[0], waypoint.y - vehicle_position[1]]))

        # Calculate lateral error as a distance between vehicle position and nearest waypoint found above
        lateral_error = np.linalg.norm([nearest_waypoint.x - vehicle_position[0], nearest_waypoint.y - vehicle_position[1]])
    ...
    def _pid_control(self, waypoints, vehicle_transform):
        ...
        e_current = self.lateral_error(waypoints, vehicle_transform)
        self._error_buffer.append(e_current)
